document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayData(dataDay, dataWeek) {
        ...
        createChart('day', 'wind-day-chart', 'du vent dans les prochaines 24h', "Heure", "Vent (km/h)", dataDay.data[2].coordinates[0], 'line', 'rgba(255, 255, 0, 1)', 'rgba(255, 255, 0, 0.2)', dataDay.data[3].coordinates[0]);
    ...
    function createChart(time, elementId, label, x, y, data, type, borderColor, backgroundColor, secondaryDataWind = null, secondaryDataTemp = null) {
        ...
        if (time === "week") labels = data.dates.map(date => new Date(date.date).toLocaleTimeString('fr-FR', { month: 'long', day: '2-digit' }));
